...
/**
 * 数字信封(SM2、SHA256、AES)，校验签名、解密过滤器
 */
@Slf4j
@Order(2)   // NationalSecretsVerifySignFilter优先执行
@WebFilter
public class VerifySignFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        // 每次拦截到请求输出访问 ip
        log.debug("数字信封过滤器 访问 IP = {}  url = {}", visitIp, req.getRequestURL());
        ...
        if (StringUtils.equalsIgnoreCase(MOTHOD_POST, method) && !contain) {
            ...
            log.debug("数字信封过滤器 post请求 parameterMap：{}", JSONObject.toJSONString(paramsMaps));

            boolean verifyResult = parsePackageParamsUtil.VerifyDataLegality(paramsMaps.get("ciphertext").toString(), paramsMaps.get("key").toString(), paramsMaps.get("iv").toString(), paramsMaps.get("hash").toString(), paramsMaps.get("sign").toString());

            log.debug("数字信封过滤器 post请求 verifyResult：{}",  verifyResult);
            if (!verifyResult) {
                ...
                String jsonBodyStr = parsePackageParamsUtil.parseFrontData(paramsMaps.get("ciphertext").toString(), paramsMaps.get("key").toString(), paramsMaps.get("iv").toString());
                log.debug("数字信封过滤器 post请求 解密后的body字符串：{}",  jsonBodyStr);
            ...
            log.debug("数字信封过滤器 get请求 parameterMap: {}", paramsMaps);

            boolean verifyResult = parsePackageParamsUtil.VerifyDataLegality(paramsMaps.get("ciphertext").toString(), paramsMaps.get("key").toString(), paramsMaps.get("iv").toString(), paramsMaps.get("hash").toString(), paramsMaps.get("sign").toString());
            log.debug("数字信封过滤器 get请求 verifyResult：{}",  verifyResult);
            if (!verifyResult) {
                ApiResponse apiResponse = new ApiResponse();
                apiResponse.setErrorCode(ErrorCode.ILLEGAL_DATA.getStatusCodeStr());
                apiResponse.setErrorMsg(ErrorCode.ILLEGAL_DATA.getStatusDesc());
                ...
                String jsonBodyStr = parsePackageParamsUtil.parseFrontData(paramsMaps.get("ciphertext").toString(), paramsMaps.get("key").toString(), paramsMaps.get("iv").toString());
                log.debug("数字信封过滤器 get请求 解密后的body字符串：{}",  jsonBodyStr);
        ...
        if (!contain) {
            ...
            responseStr = parsePackageParamsUtil.packageResponseData(responseStr);
            //把返回值输出到客户端
